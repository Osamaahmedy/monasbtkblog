...
const UsersIcon = () => (
  <svg className="w-5 h-5 text-[#FF157D] filter drop-shadow-[0_0_6px_rgba(255,21,125,0.6)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
...
const StoreButton = ({ href, icon, topLabel, bottomLabel, isComingSoon = false, isRtl }) => (
  <motion.a
    ...
    className={`
      ...
      ${isComingSoon ? 'cursor-not-allowed opacity-45' : 'cursor-pointer hover:bg-white/[0.08] hover:border-white/20'}
    `}
  >
    {!isComingSoon && (
    ...
    <div className="flex flex-col leading-tight text-left">
      <span className={`text-white/70 text-[10px] tracking-wider ${isRtl ? 'font-mikhak-regular' : 'font-outfit'}`}>
        {topLabel}
      </span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
          ...
          >
            ...
            <motion.p
              variants={itemVariants}
              className={`text-base sm:text-lg text-white/90 leading-relaxed max-w-lg ${isRtl ? 'font-mikhak-regular' : 'font-outfit font-light'}`}
            >
              {t.subtitle}
            </motion.p>
            ...
            <motion.div variants={itemVariants} className="space-y-4">
              <p className={`text-xs font-bold text-white/75 uppercase tracking-widest ${isRtl ? 'font-mikhak-bold' : 'font-outfit'}`}>
                {t.getApp}
              </p>
              ...
              {[
                ...
                <div key={stat.label} className="text-left select-none">
                  <div className="flex items-center gap-2 mb-1">
                    {stat.icon}
                    <span className={`text-white font-extrabold text-2xl ${isRtl ? 'font-mikhak-bold' : 'font-outfit'}`}>
                      {stat.value}
                    </span>
                  </div>
                  <p className={`text-white/75 text-xs font-semibold mt-1 ${isRtl ? 'font-mikhak-regular' : 'font-outfit'}`}>{stat.label}</p>
                </div>
              ))}
            </motion.div>
          </motion.div>
