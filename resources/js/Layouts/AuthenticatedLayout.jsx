...
export default function AuthenticatedLayout({ header, children }) {
    ...
    const navItems = [
        ...
        { name: 'Articles', href: route('admin.articles.index'), icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z', active: route().current('admin.articles.*') },
        { name: 'Categories', href: route('admin.categories.index'), icon: 'M4 6h16M4 12h16M4 18h16', active: route().current('admin.categories.*') },
        { name: 'Comments', href: route('admin.comments.index'), icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z', active: route().current('admin.comments.*') },
    ];

    return (
        <div className="min-h-screen bg-[#F8FAFC] font-mikhak-regular overflow-x-hidden">
            {/* Desktop Sidebar */}
            <aside 
                className={`fixed top-0 left-0 z-40 h-screen transition-all duration-300 ease-in-out bg-white border-r border-gray-100 ${isSidebarOpen ? 'w-72' : 'w-20'} hidden lg:block shadow-2xl shadow-gray-200/50`}
            >
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {isSidebarOpen && <span className="ml-4 font-mikhak-medium">{item.name}</span>}
                                </Link>
                            );
                        })}
                    </nav>
                        ...
                        <div className={`p-4 rounded-2xl bg-gradient-to-br from-primary/5 to-secondary/5 border border-white ${isSidebarOpen ? '' : 'hidden'}`}>
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="overflow-hidden">
                                    ...
                                    <p className="text-xs text-gray-400 truncate w-32">Administrator</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {isMobileMenuOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed top-0 left-0 z-50 w-72 h-screen bg-white shadow-2xl flex flex-col p-6 lg:hidden"
                        >
                            ...
                            <nav className="flex-1 space-y-2">
                                {navItems.map((item) => (
                                    ...
                                    >
                                        ...
                                        <span className="ml-4 font-mikhak-medium text-sm">{item.name}</span>
                                    </Link>
                                ))}
                            </nav>

                            <div className="pt-6 border-t border-slate-100 mt-6">
                                <div className="p-3.5 rounded-xl bg-slate-50 flex items-center space-x-3">
            ...
            {/* Main Wrapper */}
            <div className={`transition-all duration-300 ease-in-out ${isSidebarOpen ? 'lg:ml-72' : 'lg:ml-20'}`}>
                {/* Header Navbar */}
                <nav className={`sticky top-0 z-30 transition-all duration-300 px-4 lg:px-8 py-4 ${
                    ...
                    <div className="max-w-7xl mx-auto flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" d={isSidebarOpen ? "M4 6h16M4 12h16M4 18h16" : "M4 6h16M4 12h8m-8 6h16"} />
                                </svg>
                            </button>
