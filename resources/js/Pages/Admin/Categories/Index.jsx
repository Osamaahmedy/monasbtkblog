        ...
        <AuthenticatedLayout
            header={
                <div className="flex justify-between items-center">
                    <div>
                        <h2 className="font-mikhak-bold text-2xl text-slate-800 leading-tight">{t.admin.categories.title}</h2>
                        <p className="text-sm text-slate-500 font-mikhak-regular mt-1">
                            {t.admin.categories.manage}
                            {categories.total > 0 && (
                                <span className={`${lang === 'ar' ? 'mr-2' : 'ml-2'} inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mikhak-bold bg-indigo-100 text-indigo-700`}>
                                    {lang === 'ar' ? `${t.admin.categories.total} ${categories.total}` : `${categories.total} ${t.admin.categories.total}`}
                                </span>
                            )}
                        </p>
                    </div>
                            ...
                            <table className="w-full text-left rtl:text-right whitespace-nowrap">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 font-mikhak-bold text-xs uppercase tracking-wider">
                                        <th className="px-6 py-4 w-16">{t.admin.categories.colImage}</th>
                                        <th className="px-6 py-4">{t.admin.categories.colCategory}</th>
                                        <th className="px-6 py-4 text-center">{t.admin.categories.colArticles}</th>
                                        <th className="px-6 py-4 text-right rtl:text-left">{t.admin.categories.colActions}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {categories.data.map((category) => (
                                        <tr key={category.id} className="hover:bg-slate-50/60 transition-colors group">
                                            ...
                                            {/* Title */}
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-mikhak-bold text-slate-900">
                                                    {lang === 'ar' ? category.title?.ar || category.title?.en : category.title?.en || category.title?.ar}
                                                </div>
                                                <div className="text-xs font-mikhak-regular text-slate-500 mt-0.5" dir={lang === 'ar' ? 'ltr' : 'rtl'}>
                                                    {lang === 'ar' ? category.title?.en : category.title?.ar}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right rtl:text-left">
                                                <div className="flex items-center justify-end rtl:justify-start space-x-2 rtl:space-x-reverse opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Link
                                                    ...
                                                    >
                                                        <svg className={`w-3.5 h-3.5 ${lang === 'ar' ? 'ml-1' : 'mr-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <button
                                                        ...
                                                        title={t.admin.categories.deleteBtn}
                                                    >
                                                        <svg className={`w-3.5 h-3.5 ${lang === 'ar' ? 'ml-1' : 'mr-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {categories.data.length === 0 && (
                                        <tr>
                                            <td colSpan="4" className="px-6 py-16 text-center">
                                                ...
                                                <h3 className="text-sm font-mikhak-bold text-slate-900 mb-1">{t.admin.categories.noCategories}</h3>
                                                <p className="text-sm text-slate-500 font-mikhak-regular mb-4">{t.admin.categories.startByCreating}</p>
                                                <Link
                                                    href={route('admin.categories.create')}
                        ...
                        {categories.total > 0 && (
                            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-3">
                                <p className="text-xs text-slate-500 font-mikhak-regular">
                                    {lang === 'ar' ? (
                                        <span>
                                            عرض <span className="font-mikhak-bold text-slate-700">{categories.from}</span>–<span className="font-mikhak-bold text-slate-700">{categories.to}</span> من أصل <span className="font-mikhak-bold text-slate-700">{categories.total}</span> أقسام
                                        </span>
                                    ) : (
                                        <span>
