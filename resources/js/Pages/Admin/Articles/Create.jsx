...
export default function Create({ categories }) {
    ...
    useEffect(() => {
        if (data.content.en || data.content.ar) {
            const timer = setTimeout(() => {
                localStorage.setItem('article_new_draft_en', data.content.en || '');
                localStorage.setItem('article_new_draft_ar', data.content.ar || '');
    ...
    const restoreDraft = () => {
        const savedEn = localStorage.getItem('article_new_draft_en');
        const savedAr = localStorage.getItem('article_new_draft_ar');
        if (savedEn || savedAr) {
            setData('content', {
        ...
        <AuthenticatedLayout
            header={
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h2 className="font-mikhak-bold text-2xl sm:text-3xl text-slate-800 leading-tight">Create New Article</h2>
                        <p className="text-sm text-slate-500 font-mikhak-regular mt-1">Publish a new article to your blog</p>
                    </div>
                    <Link
                    ...
                    <div className="bg-white overflow-hidden shadow-sm rounded-3xl border border-slate-100 p-4 sm:p-8">
                        <form onSubmit={submit} className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    <label className="block text-sm font-mikhak-bold text-slate-700">Featured Image <span className="text-rose-500">*</span></label>
                                    <div className="mt-1 flex items-center">
                                        <label className="w-full flex flex-col items-center px-4 py-5 bg-slate-50 text-slate-400 rounded-2xl border-2 border-slate-200 border-dashed cursor-pointer hover:bg-slate-100/50 hover:border-primary transition-all">
                                            ...
                                            <span className="text-xs font-mikhak-medium text-slate-500">{data.image ? data.image.name : 'Choose Image'}</span>
                                            <input
                                                type="file"
                                                className="hidden"
                                                accept="image/*"
                            ...
                            <div className={lang === 'ar' ? 'text-right' : ''}>
                                ...
                                <textarea
                                    ...
                                    rows="2"
                                    placeholder={lang === 'en' ? 'Brief summary of the article...' : 'وصف موجز للمقالة...'}
